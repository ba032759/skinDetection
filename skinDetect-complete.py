    ...
    for i in range(0, height):
        for j in range(0, width):
            if (int(image[i, j, 0]) + int(image[i, j, 1]) + int(image[i, j, 2])) == 0:
                ...
                new_image[i, j, 0] = float(image[i, j, 2]) / (
                    int(image[i, j, 0]) + int(image[i, j, 1]) + int(image[i, j, 2]))
                new_image[i, j, 1] = float(image[i, j, 1]) / (
                    int(image[i, j, 0]) + int(image[i, j, 1]) + int(image[i, j, 2]))
    return new_image
...
print(mean_b)
print(sd_b)
img = removeNonSkinBlue(img, mean_b - sd_b * 3, mean_b + 3 * sd_b)
...
cv2.rectangle(img, (x, int(y * 0.8)), (x + w, y + h), (0, 0, 0), -1)
cv2.imshow("test", img)
